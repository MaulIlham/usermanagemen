...
func (h Handler) SaveRoleService(c *gin.Context) {

	var param string
	listService := []*models.Service{}
	...
	body, _ := ioutil.ReadAll(c.Request.Body)
	if err := json.Unmarshal(body, &listService); err != nil {
		log.Fatal("could not parse request body")
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("invalid request body: %s", err.Error())})
		return
	}

	err = newRole.InsertRoleService(listService,id)
	if err != nil {
		log.Println(err)
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf(err.Error())})
		return
	...
	c.JSON(http.StatusOK, models.Logger{
		Status: "Ok",
		Message: fmt.Sprintf("Insert Data Success"),
		Data: listService,
	})
}
...
func (h Handler) SaveRoleMenu(c *gin.Context) {
	...
	body, _ := ioutil.ReadAll(c.Request.Body)
	if err := json.Unmarshal(body, &listMenu); err != nil {
		log.Fatal("could not parse request body")
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("invalid request body: %s", err.Error())})
		return
	}

	err = newRole.InsertRoleMenu(listMenu,id)
	if err != nil {
		log.Println(err)
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf(err.Error())})
		return
...
func (h Handler) SaveRoleUser(c *gin.Context) {
	...
	body, _ := ioutil.ReadAll(c.Request.Body)
	if err := json.Unmarshal(body, &listUser); err != nil {
		log.Fatal("could not parse request body")
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("invalid request body: %s", err.Error())})
		return
	}

	err = newRole.InsertRoleUser(listUser,id)
	if err != nil {
		log.Println(err)
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf(err.Error())})
		return
